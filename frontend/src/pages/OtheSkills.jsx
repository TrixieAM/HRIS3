...
import {
    ...
    Paper, Typography, Box, AppBar, Toolbar, IconButton, CssBaseline, ThemeProvider, createTheme, useMediaQuery
  } from '@mui/material';
...
const OtherSkills = () => {
    ...
    const [newNonAcademicDistinctions, setNewNonAcademicDistinctions] = useState('');
    const [newMembershipInAssociation, setNewMembershipInAssociation] = useState('');
    ...
    const fetchSpecialSkills = async () => {
        try {
          const response = await axios.get('http://localhost:5000/otherInfo/otherinformation');
      ...
      const addSpecialSkills = async () => {
        if (!newSpecialSkills || !newNonAcademicDistinctions || newMembershipInAssociation.trim() === '') return;
      ...
      const updateSpecialSkills = async () => {
        if (!editItem || !editItem.specialSkills || !editItem.nonAcademicDistinctions || editItem.membershipInAssociation.trim() === '') return;
        await axios.put(`http://localhost:5000/otherInfo/otherinformation/${editItem.id}`, {
      ...
      const deleteSpecialSkills = async (id) => {
        await axios.delete(`http://localhost:5000/otherInfo/otherinformation/${id}`);
        fetchSpecialSkills();
      };
                  ...
                  <TableCell>
                    {editItem && editItem.id === item.id ? (
                      <TextField
                        ...
                        onChange={(e) => setEditItem({ ...editItem, specialSkills: e.target.value })}
                        style={{ minWidth: 150, marginRight: '10px' }}
                      />
                    ) : (
                      item.specialSkills
                  ...
                  <TableCell>
                    {editItem && editItem.id === item.id ? (
                      <TextField
                        ...
                        onChange={(e) => setEditItem({ ...editItem, nonAcademicDistinctions: e.target.value })}
                        style={{ minWidth: 150, marginRight: '10px' }}
                      />
                    ) : (
                  ...
                  <TableCell>
                    {editItem && editItem.id === item.id ? (
                      <TextField
                        ...
                        onChange={(e) => setEditItem({ ...editItem, membershipInAssociation: e.target.value })}
                        style={{ minWidth: 150 }}
                      />
                    ) : (
                    ...
                    {editItem && editItem.id === item.id ? (
                      <>
                        ...
                        <Button 
                          ...
                          variant="contained" 
                          style={{ backgroundColor: '#800000', color: 'white', marginRight: '10px' }}
                        >
                          <CancelIcon /> Cancel
                        </Button>
                      </>
